...
8. **Versioning**. Необходимо избегать _breaking changes_, клиенты сервиса не должны сразу каскадно меняться от его изменений. Изменения нужно вносить **постепенно**, чтобы у клиентов было время на изменение себя. Для этого необходимо _версионировать_ API сервиса. Некоторые тезисы касательно работы с изменениями:
	- Реализовывать паттерн [Tolerant Reader](https://martinfowler.com/bliki/TolerantReader.html) у потребителей сервиса, если они в зоне ответственности команды. Например, в случае если сервис возвращает XML, пусть потребитель использует XPath для чтения нужных ему нод, и теперь при необходимости реструктуризации возвращаемого XML, сервис может спойкойно это делать, главное оставить имена нод теже, что и были, а потребитель их сможет вычитать с помощью своего XPath. _Postel's Law: _**Be conservative in what you do, be liberal in what you accept from others**_.
	- Catch Breaking Changes Early. В этом помогут _consumer-driven contracts_
	- Использовать **Semantic Versioning**, с паттерном версии _**MAJOR.MINOR.PATCH**_. При инкременте _MAJOR_ - были крупные изменения, не совместимые с предыдущими версиями. При инкременте _MINOR_ - добавлена новая функциональность, которая обратно совместима с предыдущими версиями. При инкременте _PATCH_ - bug fix для существующей функциональности.
	- **Coexist Different Endpoints**. Подход хороший, но потенциально ведущий к проблемам, когда сосуществующих версий больше двух и ими становится тяжело управлять. Как вариант, можно внутри сервиса добавить логики трансормации запросов от старых Endpoints в новые.
	![Coexisting different endpoint versions](https://www.oreilly.com/library/view/building-microservices/9781491950340/assets/bdms_0405.png)
	- Использование **Multiple Concurrent Service Versions** - подход иммет право на жизнь, но он не учен хорош, т.к. появляется необходимость управлять и поддерживать два практически одинаковых сервиса. Он может применяться в ситуациях, когда цена изменений старых потребителей слишком высока, пример Netflix с его базой пользователей, которые могут использовать абсолютно разные, в т.ч. и старые устройства, которые просто могут не поддерживать новые технологии и API. Также подход может применяться для blue/green deployment, когда 2 версии живут одновременно минуты, максимум - часы.

9. UI layer за которым стоит множество микросервисов может общаться с ними следующим образом:
	* UI компонент, который шлет запросы всем необходимым сервисам
	![Using multiple APIs to present a user interface](https://www.oreilly.com/library/view/building-microservices/9781491950340/assets/bdms_0407.png)
	...
	* Каждый сервис представляет свой UI виджет. Все они собираются в некий box и показываются пользователю.
	![Services directly serving up UI components for assembly](https://www.oreilly.com/library/view/building-microservices/9781491950340/assets/bdms_0408.png)
	...
	* **API Gateway**, что по сути является **фасадом** ко всем сервисам. 
	![Using a single monolithic gateway to handle calls to/from UIs](https://www.oreilly.com/library/view/building-microservices/9781491950340/assets/bdms_0409.png)
	...
	* Альтернативой, поддерживаемой автором, является **Гибридный подход**, где каждый вид UI (mobile, web, admin panel) имеет свой **API Gateway** (backend-for-frontends)
	![Using dedicated backends for frontends](https://www.oreilly.com/library/view/building-microservices/9781491950340/assets/bdms_0410.png)
		- &#9989; сервисы и UI независимы друг от друга и развязаны через Gateway
...
10. При необходимости использования какого-либо решения, как архитектор, нужно решить стоит ли это решение писать саммим или взять/купить готовое и внедрить. Опасность готовых решений может быть в том, что оно не полностью подходит и его надо кастомизировать под себя, что бывает не так просто и может дорого стоить и, возможно, проще что-то переделать у себя, чтобы это решение начало работать "из коробки" либо все же написать самим что-то, что не такое громоздкое и гибкое для кастомизации, но подходящее идеально под нужды проекта. А в случае необходимости избавления от не контролируемого third-party инструмента из системы можно воспользоваться паттерном [Strangler](https://martinfowler.com/bliki/StranglerFigApplication.html).

11. **Prefer Choreography (event-based communication) over Orchestration (existing of some broker which redirects calls from one microservice to another and knows about all microservices participating in certain request)**
...
Еще одна причина использования кэша - **resiliency**. В случае отказа части системы, можно вернуть закэшированную версию тех данных, которую отказавший сервис возвращал.
