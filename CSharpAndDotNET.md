...
- 1) **Attached Tasks**
Parent Task Started -> Child Task Started -> Child Task Finished -> Parent Task Finished
Исключения в дочерних задачах распространяются до родителей.
Состояние родительского объекта зависит от состояния дочернего объекта.
Отмена Child приводит к отмене Parent, TaskCanceledException распространяется в ожидающий задачу поток.
  2) **Detached Tasks**
Parent Task Started -> Child Task Started -> Parent Task Finished -> Child Task Finished (or could be as in Attached)
Исключения в дочерних задачах **не** распространяются до родителей.
Состояние родительского объекта **не** зависит от состояния дочернего объекта.
Отмена Child **не** приводит к отмене Parent, TaskCanceledException **не** распространяется в ожидающий задачу поток.
