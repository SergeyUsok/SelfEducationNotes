...
**Коэффициенты влияния** можно получить также с помощью **inverse distance** и некоторых вычислений над ними. Главное, чтобы в результаты мы получили **проценты**, которые и показывают это влияние.
...
**kNN (K-nearest neighbor)** может быть также использован для классификации. Для этого достаточно найти расстояния между **уже проклассифицированным** элементом и элементом, который мы **пытаемся классифицировать**. На основании этих расстояний можно вычислить **коэффициенты влияния** (напр., с помощью inverse distance), понять к какому из соседей (neighbors) элемент ближе и взять классификацию у этого соседа.

**kNN** использется для:
- рекомендаций товаров на Amazon
- оценки риска потребительского кредитования
- распознавания лиц
- классификации земной поверхности с использованием анализа изображений
- классификации пола людей на изображениях
- рекомендаций web страниц
- рекомендаций туристических пакетов
